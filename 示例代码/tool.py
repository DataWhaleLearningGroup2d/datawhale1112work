...
from config import GOOGLE_SEARCH_SERPER_API_KEY
...
class ReactTools:
    ...
    def google_search(self, search_query: str) -> str:
        ...
        headers = {
            'X-API-KEY': GOOGLE_SEARCH_SERPER_API_KEY,
    ...
    def baidu_search(self, search_query: str) -> str:
        """执行百度搜索
        ...
        try:
            baidu_results = search(search_query, num_results=10) # returns 10 or less 
            # response = requests.request("POST", url, headers=headers, data=payload).json()
            # organic_results = response.get('organic', [])
            
            # 格式化搜索结果
            formatted_results = []
            for idx, result in enumerate(baidu_results[:5], 1):
                title = result.get('title', '无标题')
                snippet = result.get('abstract', '无描述')
                link = result.get('url', '')
                formatted_results.append(f"{idx}. **{title}**\n   {snippet}\n   链接: {link}")
...
if __name__ == "__main__":
    ...
    result = tools.baidu_search("2026年在杭州如何找到好的1人公司的商业机会？")
    print(result)